...
void save_data(char *recvbuf, int len, char *client_data, DataIndex *data);
...
{
	int num;
	char client_data[5 * c_max_data_size];
	DataIndex index;
	...
	while (num = recv(connectfd, recvbuf, c_max_data_size, 0))
	{
		...
		save_data(recvbuf, num, client_data, &index);
	...
	close(connectfd);
	std::cout << "Client(" << client_name << ") closed connection, use's data:" << client_data << std::endl;
}
...
void save_data(char *recvbuf, int len, char *client_data, DataIndex *data)
{
	...
	{
		client_data[data->index++] = recvbuf[i];
	}
	client_data[data->index] = '\0';
}
